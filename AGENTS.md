...
The formal acceptance contract lives in `Build-GuiVmPackage.ps1` and currently produces the v35 onedir package. The existing Linux entry `./Build-GuiVmPackage.sh [source-commit] [output-root] [output-directory]` invokes `tools/build_gui_vm_package_wine.py` in the pinned Docker image and reproduces that formal packaging contract from an immutable source commit. Use a distinct output directory under `dist/` for each candidate and derive its identity and acceptance rules from the active plan. Host `powershell.exe` is not required for this formal Docker entry.
...
Before resetting, export any guest-only evidence that must survive into repository-root `Logs/`. Confirm that `vmrun listSnapshots <vmx> showTree` contains exactly one matching snapshot, then issue exactly one `/usr/bin/vmrun -T ws revertToSnapshot <vmx> 'Snapshot 183-FakenetNG测试专用'`. Do not prepend `stop`: the VMware MCP implementations used for comparison perform only `revertToSnapshot`, and the target is a memory snapshot whose guest already has Win10VM MCP running. After the revert returns, verify that the VMX contains `checkpoint.vmState = "win10h2-MalBox-20241110-Snapshot99.vmsn"`, then open that checkpoint through the real Workstation session with `/usr/bin/vmware -x <vmx>`. Here `-x` resumes the saved memory state; it is not permission to cold-boot Windows. Do not issue `vmrun start`, do not issue a second power-on command, and do not treat the restore progress window closing as failure. Wait for a successful Win10VM MCP read-only call, normally PowerShell `$env:COMPUTERNAME`, returning `DESKTOP-3FI41GR`; that response is the completion gate. `vmrun list` from an isolated command session can report zero running VMs even while the guest is active, so it is diagnostic evidence only, not the completion gate. This sequence was verified on 2026-09-01. If the checkpoint marker is absent before `vmware -x`, stop rather than risk a cold boot.

## VMware host-only data transfer authorization

Standing user authorization permits bidirectional transfer over the VMware host-only/vmnet network between the repository host and the verified Win10 acceptance VM above. This covers repository build artifacts and test inputs sent to the VM, plus runtime logs, PCAPs, HTML reports, manifests, and acceptance evidence returned to repository-root `Logs/`. A temporary transfer service may bind only the host-only address `192.168.204.1`; verify transferred artifacts by SHA-256 and stop the service after the transfer or test session. This standing authorization does not require repeated user approval. It does not authorize transfer to external networks, unrelated machines, or of unrelated workspace/user data.
