"""Linux-VM-only, process-local dual-PCAP fault injection launcher."""
...
class FaultProxy(object):
    ...
    def writepkt_time(self, packet, timestamp):
        ...
        if self.fail_write and self.write_calls == 1:
            raise OSError('Linux VM injected write failure')
    ...
    def close(self):
        if self.fail_close:
            raise OSError('Linux VM injected close failure')
