...
$currentProperty=Get-ItemProperty $key -Name Environment -ErrorAction SilentlyContinue;$current=@($currentProperty.Environment);$currentPresent=$null -ne $currentProperty -and $null -ne $currentProperty.Environment;$same=if($saved.present){$currentPresent -and @(Compare-Object @($saved.values) $current).Count -eq 0}else{-not $currentPresent};@{enabled=$false;backup=$envbackup;environment_restored=$same;stop_path=$stopPath;state=(Get-Service fakenetng-mcp).Status.ToString()}|ConvertTo-Json -Compress"""
    ...
    def _run_gate(self, fault: str, nonce: str, capture: dict[str, Any]) -> dict[str, Any]:
        ...
        command = (
            ...
            "$probe=" + quote_ps(capture['probe']) + ";$launcher=" + str(int(capture['pid'])) + ";$started=[DateTimeOffset]::UtcNow;$deadline=[DateTime]::UtcNow.AddSeconds(60);$latched=$null;$seen=0;$runs=@();$runsAt=0;"
            ...
            # connection lives only ~230ms; the diverter_stop injection then
            # fired after the relay peer teardown and the conservative action
            # interval could never sit inside the session.  Parse each probe
            # line exactly once, and keep the run-directory enumeration (the
            # other per-iteration cost, hundreds of accumulated run dirs on
            # the acceptance VM) in a cache that refreshes at most every
            # 500ms while waiting and only when empty once established.
