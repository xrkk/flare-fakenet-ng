...
def parse_line(text, ref):
    ...
    if m:
        ...
        valid = re.fullmatch(r'(requested to connect|connect proceeding|connect completed|abort issued|abort completed|shutdown initiated|close issued|disconnect completed|connection terminated)\.(?: PID = (\d+)\.)?', tail)
        if not valid:
            raise ValueError('unsupported TCP lifecycle: ' + body)
        endpoint(m[2]); endpoint(m[3])
        event.update(tcb=m[1].upper(), local=m[2], remote=m[3], kind=valid[1],
                     pid=int(valid[2]) if valid[2] else None, terminal=valid[1] in END_VERBS)
        return event
